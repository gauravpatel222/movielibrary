        ...
        const moviesData: Movie[] = await Promise.all(
          playlist.imdbIDs.map(async (imdbID) => {
            const response = await axios.get<Movie>(
              `${baseUrl}/api/movies/search/find/${imdbID}`
            );
  ...
  return (
    <div className="group relative flex flex-col gap-4 p-4 border border-gray-700 shadow-lg rounded-lg bg-gradient-to-br from-gray-800 to-black text-white transform transition-transform hover:scale-105 hover:shadow-2xl mb-6">
      <div className="flex items-center gap-5 justify-between">
        <span
          className="text-2xl font-semibold cursor-pointer hover:text-blue-400 transition-colors"
      ...
      <div className="flex items-center mt-2">
        <label
          htmlFor={`visibility-toggle-${playlist.id}`}
        ...
        >
          ...
          <div
            className={`relative w-12 h-6 rounded-full transition-colors ${
              visibility ? "bg-green-500" : "bg-red-500"
            }`}
          >
            <input
            ...
            <div
              className={`dot absolute left-0 top-0 bg-white w-6 h-6 rounded-full transition-transform ${
                visibility ? "transform translate-x-6" : ""
              }`}
            ></div>
          </div>
        </label>
        ...
        <div className="mt-4">
          {loading ? (
            ...
            movies.map((movie, index) => (
              <div
                key={index}
                className="flex items-center gap-4 my-2 bg-gray-900 p-3 rounded-lg shadow-md"
              >
                <img
                  src={movie.Poster}
